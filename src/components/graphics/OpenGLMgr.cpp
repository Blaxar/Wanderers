...
void OpenGLMgr::setUp()
{
    ...
    glParams.camPosX = _pov->_spatial._default._x;
	  ...
	  /* /!\ Be careful when reversing the object rotation to adjust the point
     * of view: calling the rotate function first rotates around X, then Y, then Z.
     *
	   * If one wants to reverse such transformation, one might first rotate Z backward,
     * then Y backward, then Y backward.
     */
...
void OpenGLMgr::update(const uint32_t elapsed_time_ns, GraphicsComponent& cmp, size_t id)
{
    ...
    glUseProgram(program);
	  ...
	  vmath::mat4 mv_matrix =
        vmath::translate<float>(spatial._default._x-glParams.camPosX,
