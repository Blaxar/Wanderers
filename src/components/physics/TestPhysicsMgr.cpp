...
void TestPhysicsMgr::update(uint16_t elapsed_time, std::vector<Entity>& entities)
{
	...
	for(Entity& entity : entities)
	{
	    ...
	    *vmath::translate<float>(0, 0, WANDERER_FORWARD_STEP);
		...
		aimRotX *= (180.0 / PI);
		aimRotY *= (180.0 / PI);
		if(aimRotX > 180) aimRotX = 360-aimRotX;
		if(aimRotY > 180) aimRotY = 360-aimRotY;
		if(aimRotX < -180) aimRotX += 360;
		if(aimRotY < -180) aimRotY += 360;
		//float aimRotZ = (atan2(aimY,aimX) - atan2(next_velo[3][1],next_velo[3][0])) * (180.0 / PI);
		std::cout << aimRotX << " " << aimRotY << std::endl;
		...
		*vmath::translate<float>(entity._physics._default._velocityX*(elapsed_time/1000.0),
								 entity._physics._default._velocityY*(elapsed_time/1000.0),
								 entity._physics._default._velocityZ*(elapsed_time/1000.0));
